...
#define TINYOBJ_SSCANF_BUFFER_SIZE (4096)
    ...
    static unsigned int updateVertex(
        ...
        assert(in_positions.size() >
               static_cast<unsigned int>(3 * i.v_idx + 2));
        ...
        if (i.vn_idx >= 0) {
            normals.push_back(
                in_normals[3 * static_cast<size_t>(i.vn_idx) + 0]);
            normals.push_back(
                in_normals[3 * static_cast<size_t>(i.vn_idx) + 1]);
            normals.push_back(
                in_normals[3 * static_cast<size_t>(i.vn_idx) + 2]);
        }
    ...
    bool LoadObj(std::vector<shape_t> &shapes,        // [output]
        ...
        std::vector<char> buf(
            static_cast<size_t>(maxchars));  // Alloc enough size.
        while (inStream.peek() != -1) {
            ...
            if ((0 == strncmp(token, "usemtl", 6)) && isSpace((token[6]))) {
                ...
                bool ret =
                    exportFaceGroupToShape(shape, vertexCache, v, vn, vt,
